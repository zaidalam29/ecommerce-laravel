                                ...
                                <div class="col-md-12">
                                    ...
                                    @foreach ($order->order_details as $od)
                                        @if ($od->product->type == PRODUCT_AFFILIATE)
                                            ...
                                            <form enctype="multipart/form-data" method="POST" action="{{route('admin.digital_product_mail')}}">
                                                @csrf
                                        ...
                                        @elseif ($od->product->type == PRODUCT_DIGITAL)
                                            ...
                                            <img src="{{asset(IMG_PRODUCT_PATH. $od->product->Primary_Image)}}"  height="120" class="img-rounded mb-25" />
                                            <form enctype="multipart/form-data" method="POST" action="{{route('admin.digital_product_mail')}}">
                                                @csrf
                                                <input type="hidden" name="product_type" value="{{$od->product->type}}">
                                                <input type="hidden" name="mail_address" value="{{$bill['email']}}">
                                                <div class="input__group mb-25">
                                                    <label for="exampleInputEmail1">{{ __('Link')}}</label>
                                                    <input type="text" class="form-control" id="link" name="link" required="" value="{{is_null($od->product->digital_file) ? $od->product->digital->link : asset(PRODUCT_DIGITAL_PRODUCT.$od->product->digital_file)}}" readonly>
                                                </div>
                                                <button type="submit" class="btn btn-primary">{{ __('Send')}}</button>
                                            </form>
